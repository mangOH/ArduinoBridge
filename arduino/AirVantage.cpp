...
void AirVantageClass::startSession(const String& url, const String& pw, uint8_t avPush, storage_e storage)
{
...
void AirVantageClass::endSession(void) {
  uint8_t cmd[] = {'1'};
  bridge.transfer(cmd, 1, NULL, 0); 
}

void AirVantageClass::subscribe(const String& name) {
  uint8_t nameLen = name.length();
  uint8_t cmd[] = {'2', nameLen };
  bridge.transfer(cmd, 2, (const uint8_t*)name.c_str(), name.length(), NULL, 0);
}

void AirVantageClass::pushBoolean(const String& name, uint8_t value) {
  uint8_t nameLen = name.length();
  uint8_t cmd[] = {'3', nameLen };
  bridge.transfer(cmd, 2, (const uint8_t*)name.c_str(), name.length(), (uint8_t*)&value, sizeof(value), NULL, 0);
}

void AirVantageClass::pushInteger(const String& name, int value) {
  uint8_t nameLen = name.length();
  uint8_t cmd[] = {'4', nameLen };
  bridge.transfer(cmd, 2, (const uint8_t*)name.c_str(), name.length(), (uint8_t*)&value, sizeof(value), NULL, 0);
}

void AirVantageClass::pushFloat(const String& name, uint8_t precision, float value) {
  struct floatVal {
    uint8_t precision;
    int32_t val;
  };
...
void AirVantageClass::pushString(const String& name, const String& value) {
  uint8_t nameLen = name.length();
...
unsigned int AirVantageClass::dataAvailable() {
  uint8_t tmp[] = {'7'};
  uint8_t res[2] = {0};
...
unsigned int AirVantageClass::readMessage(uint8_t *buff, unsigned int size) {
  uint8_t tmp[] = {'8'};
  return bridge.transfer(tmp, 1, buff, size);
}

void AirVantageClass::readMessage(String &str, unsigned int maxLength) {
  uint8_t tmp[] = {'8'};
...
String AirVantageClass::parseTimestamp(const String& name, const String& msg) {
  String ret;
...
bool AirVantageClass::parseBoolean(const String& name, const String& msg) {
  bool ret = false;
...
long AirVantageClass::parseInteger(const String& name, const String& msg) {
  long ret = 0;
...
float AirVantageClass::parseFloat(const String& name, const String& msg) {
  float ret = 0.0;
...
String AirVantageClass::parseString(const String& name, const String& msg) {
  String ret;
...
AirVantageClass AirVantage(Bridge);
