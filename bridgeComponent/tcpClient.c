...
static int swi_mangoh_bridge_tcp_client_readFromSockets(swi_mangoh_bridge_tcp_client_t*);
static int swi_mangoh_bridge_tcp_client_writeToSockets(swi_mangoh_bridge_tcp_client_t*);
static int swi_mangoh_bridge_tcp_client_dropStarvingClients(swi_mangoh_bridge_tcp_client_t*);

static int swi_mangoh_bridge_tcp_client_close(swi_mangoh_bridge_tcp_client_info_t* tcpClientInfo)
{
    int32_t res = LE_OK;
    ...
    tcpClientInfo->sockFd = SWI_MANGOH_BRIDGE_TCP_CLIENT_SOCKET_INVALID;
    res = LE_OK;
...
static int swi_mangoh_bridge_tcp_client_broadcast(swi_mangoh_bridge_tcp_client_t* tcpClients, uint32_t from)
{
    int32_t res = LE_OK;
    ...
    {
        if ((idx != from) && (tcpClients->info[idx].sockFd != SWI_MANGOH_BRIDGE_TCP_CLIENT_SOCKET_INVALID))
        {
            LE_DEBUG("socket[%u](%d)", idx, tcpClients->info[idx].sockFd);
            LE_ASSERT(tcpClients->info[idx].sendBuffer != NULL);

            if (tcpClients->info[idx].sendBuffLen + tcpClients->info[from].sendBuffLen > SWI_MANGOH_BRIDGE_TCP_CLIENT_SEND_BUFFER_LEN)
            {
...
static int swi_mangoh_bridge_tcp_client_readFromSockets(swi_mangoh_bridge_tcp_client_t* tcpClients)
{
    int32_t res = LE_OK;

    LE_ASSERT(tcpClients);

    tcpClients->maxSockFd = SWI_MANGOH_BRIDGE_TCP_CLIENT_SOCKET_INVALID;
    FD_ZERO(&tcpClients->readfds);

    uint32_t idx = 0;
    for (idx = 0; idx < SWI_MANGOH_BRIDGE_TCP_CLIENT_MAX_CLIENTS; idx++)
    {
        if (tcpClients->info[idx].sockFd != SWI_MANGOH_BRIDGE_TCP_CLIENT_SOCKET_INVALID)
        {
    ...
    if (tcpClients->maxSockFd == SWI_MANGOH_BRIDGE_TCP_CLIENT_SOCKET_INVALID)
    {
        goto cleanup;
    }
        ...
        for (idx = 0; idx < SWI_MANGOH_BRIDGE_TCP_CLIENT_MAX_CLIENTS; idx++)
        {
                ...
                (FD_ISSET(tcpClients->info[idx].sockFd, &tcpClients->readfds)))
            {
                ...
                int32_t bytesRead = recv(tcpClients->info[idx].sockFd, tcpClients->info[idx].rxBuffer + tcpClients->info[idx].recvBuffLen,
                                         SWI_MANGOH_BRIDGE_TCP_CLIENT_RECV_BUFFER_LEN - tcpClients->info[idx].recvBuffLen, 0);
                if (bytesRead < 0)
                {
                    LE_ERROR("ERROR socket[%u](%d) recv() failed(%d/%d)", idx, tcpClients->info[idx].sockFd, bytesRead, errno);

                    res = swi_mangoh_bridge_tcp_client_close(&tcpClients->info[idx]);
                    if (res != LE_OK)
                    {
                        LE_ERROR("ERROR swi_mangoh_bridge_tcp_client_close() failed(%d)", res);
                        goto cleanup;
                    }
                ...
                else if ((bytesRead == 0) && (SWI_MANGOH_BRIDGE_TCP_CLIENT_RECV_BUFFER_LEN - tcpClients->info[idx].recvBuffLen > 0))
                {
                ...
                else
                {
                    ...
                    {
                        res = swi_mangoh_bridge_tcp_client_broadcast(tcpClients, idx);
                        if (res)
                        {
                            LE_ERROR("ERROR swi_mangoh_bridge_tcp_client_broadcast() failed(%d)", res);
                            goto cleanup;
                        }
                    }
                }
...
static int swi_mangoh_bridge_tcp_client_writeToSockets(swi_mangoh_bridge_tcp_client_t* tcpClient)
{
    int32_t res = LE_OK;
    ...
    tcpClient->maxSockFd = SWI_MANGOH_BRIDGE_TCP_CLIENT_SOCKET_INVALID;

    uint32_t idx = 0;
    for (idx = 0; idx < SWI_MANGOH_BRIDGE_TCP_CLIENT_MAX_CLIENTS; idx++)
    {
        if (tcpClient->info[idx].sockFd != SWI_MANGOH_BRIDGE_TCP_CLIENT_SOCKET_INVALID)
        {
    ...
    if (tcpClient->maxSockFd == SWI_MANGOH_BRIDGE_TCP_CLIENT_SOCKET_INVALID)
    {
        goto cleanup;
    }
                ...
                (FD_ISSET(tcpClient->info[idx].sockFd, &tcpClient->writefds)))
            {
                ...
                if (tcpClient->info[idx].sendBuffLen > 0)
                {
                    ...
                    if (bytesSent < 0)
                    {
                        ...
                        res = swi_mangoh_bridge_tcp_client_close(&tcpClient->info[idx]);
                        if (res != LE_OK)
                        {
                            LE_ERROR("ERROR swi_mangoh_bridge_tcp_client_close() failed(%d)", res);
                            goto cleanup;
...
static int swi_mangoh_bridge_tcp_client_dropStarvingClients(swi_mangoh_bridge_tcp_client_t* tcpClient)
{
    int32_t res = LE_OK;
    ...
    {
        if ((tcpClient->info[idx].sockFd != SWI_MANGOH_BRIDGE_TCP_CLIENT_SOCKET_INVALID) &&
            (tcpClient->info[idx].sendBuffLen == SWI_MANGOH_BRIDGE_TCP_CLIENT_SEND_BUFFER_LEN))
        {
...
int swi_mangoh_bridge_tcp_client_getReceivedData(swi_mangoh_bridge_tcp_client_t* tcpClient, int8_t* buff, uint32_t* len, uint32_t maxLen)
{
    int32_t res = LE_OK;
...
int swi_mangoh_bridge_tcp_client_write(swi_mangoh_bridge_tcp_client_t* tcpClient, const uint8_t* buff, uint32_t len)
{
    int32_t res = LE_OK;
    ...
    for (idx = 0; idx < SWI_MANGOH_BRIDGE_TCP_CLIENT_MAX_CLIENTS; idx++)
    {
        if (tcpClient->info[idx].sockFd != SWI_MANGOH_BRIDGE_TCP_CLIENT_SOCKET_INVALID)
        {
            ...
            if (len + tcpClient->info[idx].sendBuffLen > SWI_MANGOH_BRIDGE_TCP_CLIENT_SEND_BUFFER_LEN)
            {
...
void swi_mangoh_bridge_tcp_client_connected(const swi_mangoh_bridge_tcp_client_t* tcpClient, int8_t* result)
{
    LE_ASSERT(tcpClient);
    LE_ASSERT(result);
    ...
    if (idx == SWI_MANGOH_BRIDGE_TCP_CLIENT_MAX_CLIENTS)
    {
        *result = false;
    }
}

int swi_mangoh_bridge_tcp_client_run(swi_mangoh_bridge_tcp_client_t* tcpClient)
{
    int32_t res = LE_OK;

    LE_ASSERT(tcpClient);

    res = swi_mangoh_bridge_tcp_client_readFromSockets(tcpClient);
    if (res != LE_OK)
    {
        LE_ERROR("ERROR swi_mangoh_bridge_tcp_client_readFromSockets() failed(%d)", res);
        goto cleanup;
    }

    res = swi_mangoh_bridge_tcp_client_writeToSockets(tcpClient);
    if (res != LE_OK)
    {
        LE_ERROR("ERROR swi_mangoh_bridge_tcp_client_writeToSockets() failed(%d)", res);
        goto cleanup;
    }

    res = swi_mangoh_bridge_tcp_client_dropStarvingClients(tcpClient);
    if (res != LE_OK)
    {
        LE_ERROR("ERROR swi_mangoh_bridge_tcp_client_dropStarvingClients() failed(%d)", res);
        goto cleanup;
    }
...
void swi_mangoh_bridge_tcp_client_setNextId(swi_mangoh_bridge_tcp_client_t* tcpClients)
{
    LE_ASSERT(tcpClients);
    tcpClients->nextId = (tcpClients->nextId + 1) % SWI_MANGOH_BRIDGE_TCP_CLIENT_MAX_CLIENTS;
};

void swi_mangoh_bridge_tcp_client_init(swi_mangoh_bridge_tcp_client_t* tcpClient, bool broadcast)
{
    LE_ASSERT(tcpClient);
    ...
    for (idx = 0; idx < SWI_MANGOH_BRIDGE_TCP_CLIENT_MAX_CLIENTS; idx++)
    {
        tcpClient->info[idx].sockFd = SWI_MANGOH_BRIDGE_TCP_CLIENT_SOCKET_INVALID;
    }
}

int swi_mangoh_bridge_tcp_client_destroy(swi_mangoh_bridge_tcp_client_t* tcpClient)
{
    int32_t res = LE_OK;
